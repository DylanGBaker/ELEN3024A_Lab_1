...
title("Graph showing the magnitude spectrum of X(f)")
xlabel("Frequency(Hz)")
ylabel("|X(f)|")
xlim([-20 20])
...
figure(1)
plot(f,abs(Mf_spect)/(length(Mf_spect) - 1))
title("Graph showing the magnitude spectrum of the message signal")
xlabel("Frequency(Hz)")
ylabel("|M(f)|")
xlim([-10 10])
